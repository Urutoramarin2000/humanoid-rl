...
# 步骤1: 加载JSON文件
with open('/home/zhengde.ma/humanoid-rl/humanoid/scripts/data/torque.json', 'r') as file:
    torque_data = json.load(file)  # 这应该是一个双层数组，包含12个子数组   # motor_velocity_data
# torque_mujoco
# torque
with open('/home/zhengde.ma/humanoid-rl/humanoid/scripts/data/dof_pos_list.json', 'r') as file:
    dof_pos_data = json.load(file)  # 这应该是一个双层数组，包含12个子数组
# dof_pos_list_mujoco
# dof_pos_list
with open('/home/zhengde.ma/humanoid-rl/humanoid/scripts/data/actions_data.json', 'r') as file:
    actions_data = json.load(file)  # 这应该是一个双层数组，包含12个子数组
# action_data_mujoco
# actions_data
with open('/home/zhengde.ma/humanoid-rl/humanoid/scripts/data/ref_pos_list.json', 'r') as file:
    ref_pos_data = json.load(file)  # 这应该是一个双层数组，包含12个子数组

with open('/home/zhengde.ma/humanoid-rl/humanoid/scripts/data/motor_velocity_data.json', 'r') as file:
    motor_velocity_data = json.load(file)  # 这应该是一个双层数组，包含12个子数组

with open('/home/zhengde.ma/humanoid-rl/humanoid/scripts/data/base_euler_xyz_data.json', 'r') as file:
    base_euler_xyz_data = json.load(file)  # 这应该是一个双层数组，包含12个子数组
# 转置数据，以便按列绘制
...
base_euler_xyz_data_transposed = list(zip(*base_euler_xyz_data))
...
num_columns_base_euler_xyz_data = len(base_euler_xyz_data_transposed)
num_rows_base_euler_xyz_data = (num_columns_base_euler_xyz_data + 1) // 2  # 确保有足够的行来容纳所有的列
...
for i, column in enumerate(torque_data_transposed):
    ...
    plt.plot([x / 100 for x in range(len(column))], column, label=f'Torque Curve {i+1}')
    plt.legend()
...
#     plt.plot([x / 100 for x in range(len(column))], column # , label=f'Ref Pos Curve {i+1}'
#             )
#     plt.legend()
...
for i, column in enumerate(dof_pos_data_transposed):
    ...
    plt.plot([x / 100 for x in range(len(column))], column )# , label=f'DOF Pos Curve {i+1}'
...
for i, column in enumerate(actions_data_transposed):
    ...
    plt.plot([x / 100 for x in range(len(column))], column)#, label=f'Action Data Curve {i+1}')
    plt.legend()
...
for i, column in enumerate(motor_velocity_data_transposed):
    plt.subplot(num_rows_ref_pos, 2, i+1)  # 创建子图
    plt.plot([x / 100 for x in range(len(column))], column  , label=f'Motor Velocity Curve {i+1}'
            )
    plt.legend()
    plt.title(f'Motor Velocity Curve {i+1} - Time Series Data')

plt.figure(figsize=(15, max(num_columns_base_euler_xyz_data, num_rows_base_euler_xyz_data)))  # 调整图表大小以容纳所有子图
for i, column in enumerate(base_euler_xyz_data_transposed):
    plt.subplot(num_rows_base_euler_xyz_data, 2, i+1)  # 创建子图
    plt.plot([x / 100 for x in range(len(column))], column  , label=f'base_euler_xyz_data Curve {i+1}'
            )
    plt.legend()
    plt.title(f'base_euler_xyz_data Curve {i+1} - Time Series Data')
