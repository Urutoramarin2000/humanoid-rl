...
def run_policy(policy, cfg, observation_data):
    ...
    #num_actions =12
    #frame_stack = 15
    #num_single_obs = 47 #[command_input(5), q(12), dq(12), actions(12), base_ang_vel(3), base_lin_vel(3)]
    ...
    while(1):   
        ...
        q, dq= [], []
        for i in range(12):
            q.append(humanoid_data.joint_state.pos[i])  # 不知道joint_state的顺序
        ...
        #将数据放入obs
        obs[0, 0] = math.sin(2 * math.pi * count_lowlevel *  cfg.sim_config.dt  / 0.5)  # 问题：0.64是cycle time，网路训练时，参数为0.5
...
def send_action(publish_data):    
    while True:
        with lock:
            current_action_data = last_action_data
...
if __name__ == '__main__':
    ...
    parser.add_argument('--terrain', action='store_true', help='terrain or plane')  # real 这部分好像没用吧
