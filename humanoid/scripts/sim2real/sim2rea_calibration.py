...
def run_policy(cfg, publish_data, calibration_number):
    ...
    #sim_dt = 0.001 
    with open('/home/root/msg_single/actions_data_softclip_filtered.json', 'r') as file:
    ...
    for action in action_data: 
        ...
        if len(action) == 12:
            ...
            mask = np.zeros_like(action_cmd, dtype=bool)
            mask[calibration_number] = 1
            action_cmd *= mask
            action_cmd = action_cmd.tolist()
            last_action_cmd *= mask
            last_action_cmd = last_action_cmd.tolist()
            publish_data.PublishAction(action_cmd, last_action_cmd)
...
if __name__ == '__main__':
    ...
    calibration_number = 9
    run_policy(Sim2RealCfg(), publish_data, calibration_number)
