#!/usr/bin/python3
#-*-coding: UTF-8 -*-
from msg_py import imu_pb2,joystick_pb2,robot_pb2,chassis_pb2,robot_pb2
import crpilot
import pycrmw
import time
import sys,os
...
class RLPublish:
  def __init__(self) -> None:
    ...
    self.action = robot_pb2.RLAction()
    """
    action => joint_num, control_mode, cmd([0,0,0..,0]), sequence, timestamp, time_step, running_flag
    """
    self.action.joint_num = 12 # 12个关节电机
    self.action.control_mode = robot_pb2.INTERPOLATE_MODE
    for i in range(12):
      #这个是个list？
  ...
  def PublishAction(self, data):
    #可改发布话题的条件
    #while send_flag:
    self.action.timestamp = int(time.time()*1e9)
    self.action.sequence += 1
    self.action.cmd = data # 12维数据 赋值，此处省略
    self.action_writer.Write(self.action)
    self.last_timestamp = self.action.timestamp
